...
public class GameBoard extends JPanel {
    ...
    /**
     * Helper method to draw the dots or pellets onto the maze. This is a void method and
     * does not return any value. The method takes in 'g2' a Graphics2D object to allow
     * the pellets or dots to be drawn on the tileGrid. The method traverses tileGrid through
     * a 2D array, checking each vertex with path tiles and draws corresponding pellet, dot, or nothing.
     */
    private void paintDotsAndPelletsHelper(Graphics2D g2) {
        for (int x = 0; x < tileGrid.length; x++) { // nested for loop traversing all tiles
            for (int y = 0; y < tileGrid[0].length; y++) {
                MazeGraph.MazeVertex v = model.graph().closestTo(x, y); // get vertex at location x,y
                MazeGraph.IPair loc = v.loc(); // saving coordinates of the vertex
                if (loc.i() != x || loc.j() != y) {
                    continue; // this tile is a wall tile, skips drawing and goes to next iteration
                }

                GameModel.Item item = model.itemAt(v);  // store if it is a dot, pellet, none
                if (item == GameModel.Item.NONE)
                    continue; // skip go to next iteration
                double d = (item == GameModel.Item.DOT ? 0.3 : 0.7); //figure out if dot or pellet

                double i = x + 0.5 - d / 2; //centering
                double j = y + 0.5 - d / 2;

                g2.setColor(Color.WHITE);
                g2.fill(new Ellipse2D.Double(i, j, d, d));
